...
import NEW_USER_JOINED_FORM_SCHEMA from './helpers/NewUserJoined';
...
function NewUserJoined() {
  ...
  const { data: content, refetch } = useGetNotificationContentQuery(
    {
      params: {
        type: NOTIFICATION_TYPE.NEW_USER_JOINED,
      },
    },
    {
  ...
  return (
    <CustomCardWrapper>
      <CustomForm
        id={String(NOTIFICATION_TYPE.NEW_USER_JOINED)}
        className="row"
        formData={NEW_USER_JOINED_FORM_SCHEMA}
        onSubmit={onSubmit}
