...
import { transformReferralHistoryResponse } from '../../helpers/utils';
...
export default function ReferalHistoryList({
  search = '',
  setSearch = () => {},
  currentPage = 0,
  setCurrentPage = () => {},
  ...
  const columns = useMemo(() => {
    return referralHistoryColumn;
  }, [currentTab]);

  const refetchData = useCallback(() => {
    try {
      if (refetchUserReferralHistory) {
  ...
  useEffect(() => {
    const transformFunction = transformReferralHistoryResponse;

    if (transformFunction) {
      const data = transformFunction(userReferralHistory);

      setTableData(data as unknown as { data: never[]; count: number });
    } else {
      setTableData({ data: [], count: 0 });
    }
  }, [currentTab, userReferralHistory]);
